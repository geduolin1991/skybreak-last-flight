...
namespace Skybreak {public partial class SkyGame {
 ...
 float BossDamageScale(Hostile e)=>e.boss?(counterWindow>0?1.65f:BossModuleCount>0?.28f:e.coreExpose>0?1.65f:1):1;
 void InitBossEncounter(){ClearCounterplay();bossMechanisms=Boss.go.AddComponent<SkyBossMechanisms>();bossMechanisms.Initialize();bossTell=bossPhaseBreak=0;bossShots=0;Boss.attackClock=1.3f;Boss.bossAttackIndex=-1;Boss.coreExpose=0;
 ...
 void ClearBossEncounter(){ClearCounterplay();bossMechanisms=null;if(coreShield)Destroy(coreShield.gameObject);bossTell=bossPhaseBreak=0;bossShots=0;bossOrder="";}
 ...
 void TickBossEncounter(Hostile e,int phase,float dt){TickCounterplay(dt);if(bossMechanisms)bossMechanisms.Tick(e.age,phase,e.coreExpose>0,bossTell>0);e.coreExpose=Mathf.Max(0,e.coreExpose-dt);if(coreShield){coreShield.enabled=BossModuleCount>0&&counterWindow<=0;coreShield.transform.localRotation=Quaternion.Euler(Mathf.Sin(e.age)*20,0,Mathf.Cos(e.age*.7f)*20);coreShield.transform.localScale=Vector3.one*(1+Mathf.Sin(e.age*4)*.045f);}
 ...
 void DrawBossMechanic(){if(Boss==null||State!=FlightState.Playing)return;string state=counterWindow>0?"民用信号反制 · 核心伤害 ×1.65":BossModuleCount>0?"护盾在线 · 剩余武装 "+BossModuleCount:Boss.coreExpose>0?"核心暴露 · 伤害 ×1.65  "+Boss.coreExpose.ToString("F1")+"s":"核心失去保护";Label(state,430,84,740,29,17,BossModuleCount>0?Art.Cyan:Art.Orange,TextAnchor.MiddleCenter);
  Label(BossInstruction,394,120,812,38,bossTell>0?21:16,bossTell>0?Art.Orange:paper,TextAnchor.MiddleCenter);if(bossTell>0)Bar(600,161,400,bossTell/(Difficulty==0?1.35f:1.05f),Art.Orange,3);
