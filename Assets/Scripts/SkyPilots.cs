...
public partial class SkyGame {
 ...
 if(!eliteSent&&StageTime>=EliteArrivalTime&&StageTime<chapterBeats[Stage][3]&&stageEndClock==0){eliteSent=true;eliteTarget=SpawnEnemy(8,new Vector3(0,1,18),4);eliteTarget.elite=true;eliteTarget.hp=eliteTarget.maxHp=360+Stage*150;eliteTarget.go.transform.localScale=Vector3.one*.75f;MissionSay("elite_arrive",82,()=>eliteTarget!=null&&Enemies.Contains(eliteTarget)&&!eliteTarget.retreating);Toast("可选目标 · 截获干扰指挥机",4);Sound("Warning",.5f);}
 }
 ...
 if(cutinClock>0&&State==FlightState.Playing){float a=Mathf.Clamp01(cutinClock/.25f);float enter=Mathf.Clamp01((1.65f-cutinClock)*5);float x=Mathf.Lerp(1700,1010,1-Mathf.Pow(1-enter,3));Rect(x,490,553,144,new Color(.005f,.012f,.026f,.9f*a));Portrait(Ship,x+175,420,368,250,true);Label("LINK BURST",x+22,508,284,26,12,accent);Label(cutinIsOverdrive?FrameName+" · 展开":ActiveSkillName,x+22,550,342,49,29,paper);Line(x+22,617,480,accent);}
 }
