...
public partial class SkyGame {
 ...
 public void UsePilotSkill(){if(State!=FlightState.Playing||SkillCooldown>0||stageEndClock>0)return;SkillCooldown=(Ship==0?18:Ship==1?22:24)*Mathf.Pow(.8f,temporalLevel)*(1-researchSkill*.05f);SkillTime=Ship==0?2:Ship==1?5:4.5f;cutinClock=1.65f;cutinIsOverdrive=false;invuln=Mathf.Max(invuln,1.2f);Sound("Overdrive",.8f);PlayPilotBark("skill",Ship,70);shake=.28f;Shockwave(PlayerPos,Ship==1?Art.Orange:Ship==2?Art.Violet:Art.Cyan,8);pilotSpeaker=Ship;
