...
public partial class SkyGame {
 ...
 void ResetStageFeatures(){eliteSent=false;sideObjectiveComplete=false;eliteTarget=null;SkillCooldown=Mathf.Max(0,SkillCooldown-8);pilotSpeaker=Ship;foreach(var s in shocks)if(s.line)Destroy(s.line.gameObject);shocks.Clear();}
 ...
 if(!eliteSent&&StageTime>=52&&StageTime<115&&stageEndClock==0){eliteSent=true;eliteTarget=SpawnEnemy(3,new Vector3(0,1,18),4);eliteTarget.elite=true;eliteTarget.hp=eliteTarget.maxHp=620+Stage*180;eliteTarget.go.transform.localScale=Vector3.one*.75f;pilotSpeaker=(Ship+1)%3;RadioName=PilotNames[pilotSpeaker]+" / 小队通讯";RadioText=new[]{"前方是干扰指挥机！击落它，我就能把撤离信号送出去。","城市的供电钥匙就在那台重装机里。别让它跑了！","它正在覆盖人类识别码。帮我争取一次重新连接的机会。"}[Stage];dialogClock=8;Toast("可选目标 · 截获干扰指挥机",4);Sound("Warning",.5f);}
 if(StageTime>=28&&StageTime<28+dt){pilotSpeaker=(Ship+2)%3;RadioName=PilotNames[pilotSpeaker]+" / 小队通讯";RadioText=new[]{"船队开始移动了。每一盏灯，都是一个等着回家的人。","小时候，我以为城市的灯永远不会熄灭……所以这次一定要赢。","天环听得到我。我知道它还记得，我们曾一起看过星星。"}[Stage];dialogClock=7;}
 }
 ...
 public void UsePilotSkill(){if(State!=FlightState.Playing||SkillCooldown>0||stageEndClock>0)return;SkillCooldown=(Ship==0?18:Ship==1?22:24)*Mathf.Pow(.8f,temporalLevel)*(1-researchSkill*.05f);SkillTime=Ship==0?2:Ship==1?5:4.5f;cutinClock=1.65f;cutinIsOverdrive=false;invuln=Mathf.Max(invuln,1.2f);Sound("Overdrive",.8f);shake=.28f;Shockwave(PlayerPos,Ship==1?Art.Orange:Ship==2?Art.Violet:Art.Cyan,8);pilotSpeaker=Ship;
 if(Ship==0){for(int i=0;i<18;i++){float a=i*Mathf.PI*2/18;AddBullet(PlayerPos,new Vector3(Mathf.Sin(a)*16,0,Mathf.Cos(a)*16),true,0,22,.16f,true);}CancelBullets(true);}
 else if(Ship==1){foreach(var e in Enemies.ToArray())if(e.pos.z>PlayerPos.z)DamageEnemy(e,e.boss?200:120);CancelBullets(true);}
 else {for(int i=-2;i<=2;i++)AddBullet(PlayerPos+new Vector3(i*.3f,0,0),Vector3.forward*24,true,2,30,.15f,true);}
 }
 void RewardElite(Hostile e){if(e.elite){rescueSignals++;sideObjectiveComplete=true;eliteTarget=null;Score+=3500;SpawnSupply(e.pos,1);SpawnSupply(e.pos+Vector3.right,0);Energy=Mathf.Min(100,Energy+30);Toast("目标完成  +3,500  /  支援补给已投放",4);pilotSpeaker=(Ship+1)%3;RadioName=PilotNames[pilotSpeaker]+" / 小队通讯";RadioText="漂亮！信号恢复了。你的侧翼交给我。";dialogClock=5;}if(leechLevel>0&&Kills%18==0){Hull=Mathf.Min(MaxHull,Hull+leechLevel);Toast("战场回收 · 装甲已修复");}}
 ...
 void DrawRadio(){if(dialogClock<=0)return;Panel(26,318,222,287);Portrait(pilotSpeaker,34,325,205,141,true,false);Rect(26,462,222,143,new Color(.004f,.009f,.016f,.98f));Label(RadioName,41,475,193,27,13,accent);if(radioLast!=RadioText){radioLast=RadioText;radioBegan=Time.unscaledTime;}int count=Mathf.Min(RadioText.Length,Mathf.Max(1,(int)((Time.unscaledTime-radioBegan)*25)));string text=RadioText.Substring(0,count);Label(text,41,513,190,83,15,paper);}
 void DrawFeatures(){if(State==FlightState.Playing){Panel(1352,289,222,124);Label("驾驶员技  [Q]",1370,304,190,24,13,muted);Label(SkillNames[Ship],1370,338,190,31,21,paper);Label(SkillCooldown>0?Mathf.CeilToInt(SkillCooldown)+" 秒":"READY TO CAST",1371,382,187,23,12,SkillCooldown>0?muted:accent);DrawRadio();if(toastClock>0)Label(toastText,432,180,736,40,18,accent,TextAnchor.MiddleCenter);}
 if(cutinClock>0&&State==FlightState.Playing){float a=Mathf.Clamp01(cutinClock/.25f);float enter=Mathf.Clamp01((1.65f-cutinClock)*5);float x=Mathf.Lerp(1700,1010,1-Mathf.Pow(1-enter,3));Rect(x,490,553,144,new Color(.005f,.012f,.026f,.9f*a));Portrait(Ship,x+175,420,368,250,true);Label("LINK BURST",x+22,508,284,26,12,accent);Label(cutinIsOverdrive?"天翼机甲展开":SkillNames[Ship],x+22,550,342,49,29,paper);Line(x+22,617,480,accent);}
 }
