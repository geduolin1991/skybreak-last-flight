...
public partial class SkyGame {
 IEnumerator VerifyMobile18(Action<bool,string> check){
  ...
  WebMobileViewport("{\"x\":0,\"y\":0.2,\"w\":1,\"h\":0.8}");check(Mathf.Abs(Cam.rect.y-.2f)<.001f,"Portrait radio dock reserves space outside the actual camera");
  WebMobileViewport("{\"x\":0,\"y\":0.9,\"w\":1,\"h\":0.8}");check(Mathf.Abs(Cam.rect.y-.2f)<.001f,"Invalid viewport cannot clip the battle beyond the canvas");
  WebMobileViewport("{\"x\":0,\"y\":0,\"w\":1,\"h\":1}");check(Cam.rect==new Rect(0,0,1,1),"Landscape restores the full battle viewport");
