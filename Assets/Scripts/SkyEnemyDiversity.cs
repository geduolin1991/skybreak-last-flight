...
public partial class SkyGame {
 ...
 void ConfigureEnemy(Hostile e){
  if(e.kind==9){e.visualOffset=new Vector3(0,-2.8f,1.6166f);e.fire=3.4f;e.marker=Art.Ring(e.go.transform,1.12f,new Color(1,.3f,.1f,.45f),.045f);e.marker.transform.localPosition=new Vector3(0,4.9f,2.79f);}
 ...
 bool TickSpecialEnemy(Hostile e,float dt){
  ...
  if(e.kind==9){
   ...
   if(e.missionIndex>=0&&e.missionIndex<World.Civilians.Count){var c=World.Civilians[e.missionIndex];Vector3 target=c.station;target.y=1;target.z-=1.73f;e.pos=Vector3.Lerp(e.origin,target,Mathf.Clamp01(e.age/3.1f));e.visualOffset=Vector3.up*Mathf.Sin(Mathf.Clamp01(e.age/3.1f)*Mathf.PI)*1.8f;if(e.marker){e.marker.transform.position=target;e.marker.transform.localScale=Vector3.one*(.7f+e.fire*.18f);}}
   ...
   else if(e.kind==7){if(e.fire<=0&&e.pos.z<13){if(Enemies.FindAll(x=>x.kind==10).Count<8){var mine=SpawnEnemy(10,e.pos+Vector3.back*.6f,0);mine.fire=4.1f;}e.fire=2.6f;}}
 ...
 void RemoveSpecialEnemy(Hostile e){if(e.marker&&e.marker.transform.parent==null)Destroy(e.marker.gameObject);if(e.go)Destroy(e.go);Enemies.Remove(e);}
