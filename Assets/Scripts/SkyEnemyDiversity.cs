...
public partial class SkyGame {
 ...
 float WardDamageScale(Hostile e){if(e.boss||e.owner!=null||e.kind>=9||e.kind==5)return 1;foreach(var ward in Enemies)if(ward.kind==5&&!ward.retreating&&(ward.pos-e.pos).sqrMagnitude<8.2f){if(e.flash<=0)Shockwave(e.pos,Art.Violet,1,.15f);return .4f;}return 1;}
 bool TickSpecialEnemy(Hostile e,float dt){
  ...
  if(e.kind==9){
   ...
   if(e.kind==4&&e.missionIndex>=0){
    if(e.pos.z<=9&&e.attackClock==0){e.attackClock=1;e.fire=3.5f;MissionSay("convoy_bomb_lock",94,()=>Enemies.Contains(e)&&!e.retreating,false);}
