...
public partial class SkyGame {
 ...
 void TickFeedback(float dt){TickNova(dt);
  ...
  for(int i=deathClouds.Count-1;i>=0;i--){var d=deathClouds[i];d.clock-=dt;if(d.clock>0)continue;var p=d.pos+new Vector3(Random.Range(-d.size,d.size),Random.Range(0,.9f),Random.Range(-d.size,d.size));Burst(p,24,1,d.size*.46f);Shockwave(p,Art.Orange,d.size*1.3f,.35f);shake=Mathf.Max(shake,.22f);if(d.bursts%2==0)Sound("Explosion",.32f);d.bursts--;d.clock=.14f;if(d.bursts<=0)deathClouds.RemoveAt(i);}
 }
 ...
 void RenderEnemyFeedback(Hostile e){
  ...
  if(flash<=0&&!e.feedbackActive)return;e.feedbackActive=flash>0;
