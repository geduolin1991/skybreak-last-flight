...
public partial class SkyWorld:MonoBehaviour {
 ...
 public void SetStage(int stage){ResetLivingWorld();coastAnchors.Clear();coastTrafficLimits.Clear();CoastFootprints=0;ocean=null;landmarkMaterials.Clear();var randomState=Random.state;Stage=stage;SectorRenderersBefore=SectorRenderersAfter=0;missionProgress=0;signalRecovered=false;foreach(var t in chunks)if(t)Destroy(t.gameObject);chunks.Clear();if(terrain)Destroy(terrain.gameObject);terrain=new GameObject("Environment · "+stage).transform;terrain.SetParent(transform);owned=terrain.gameObject.AddComponent<SkyOwnedResources>();motion=0;
 ...
 void Update(){float dt=Mathf.Min(Time.deltaTime,.05f);if(!Scrolling)return;motion+=dt*Speed;UpdateChapterAtmosphere(dt);TickGroundLife(dt);UpdateRescueWakes();if(ocean)ocean.SetFloat("_Travel",motion);foreach(var t in chunks){t.position+=Vector3.back*dt*Speed;if(t.position.z<-45)t.position+=Vector3.forward*140;}UpdateCoast();}
}
[ExecuteAlways]
