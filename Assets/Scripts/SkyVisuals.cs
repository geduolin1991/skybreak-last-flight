...
public partial class SkyWorld:MonoBehaviour {
 ...
 var fill=new GameObject("Warm bounce").AddComponent<Light>();fillLight=fill;fill.type=LightType.Directional;fill.color=new Color(1,.43f,.22f);fill.intensity=.45f;fill.transform.rotation=Quaternion.Euler(22,145,0);
 RenderSettings.ambientMode=AmbientMode.Trilight;RenderSettings.ambientSkyColor=new Color(.15f,.23f,.32f);RenderSettings.ambientEquatorColor=new Color(.065f,.1f,.15f);RenderSettings.ambientGroundColor=new Color(.035f,.07f,.1f);RenderSettings.fog=true;RenderSettings.fogMode=FogMode.ExponentialSquared;RenderSettings.fogDensity=.0045f;SetStage(0);
 }
 public void SetStage(int stage){ResetLivingWorld();ResetCampaignWorld();coastAnchors.Clear();coastTrafficLimits.Clear();framedCoast.Clear();coastMeshes.Clear();coastInset=-1;CoastFootprints=0;ocean=null;landmarkMaterials.Clear();var randomState=Random.state;Stage=stage;SectorRenderersBefore=SectorRenderersAfter=0;missionProgress=0;signalRecovered=false;foreach(var t in chunks)if(t)Destroy(t.gameObject);chunks.Clear();if(terrain)Destroy(terrain.gameObject);terrain=new GameObject("Environment · "+stage).transform;terrain.SetParent(transform);owned=terrain.gameObject.AddComponent<SkyOwnedResources>();motion=0;
 Cam.backgroundColor=stage==2?new Color(.008f,.014f,.038f):stage==0?new Color(.095f,.19f,.29f):new Color(.08f,.18f,.24f);RenderSettings.fogColor=Cam.backgroundColor;SetChapterLighting();
 ...
 }else if(stage==2){CreateStars();BuildOrbitalDepth115();}
 Random.InitState(120+stage);
 for(int i=0;i<10;i++){var t=new GameObject("Scenery sector "+i).transform;t.SetParent(terrain);t.position=new Vector3(0,0,-35+i*14);chunks.Add(t);if(stage==0)Harbor(t,i);else if(stage==1)City(t,i);else Orbit115(t,i);AddChapterLandmark(t,i);CreateGroundDetail(t,i);BuildDistrictTransit(t,i);BuildCampaignSector(t,i);if(stage==2)ToneOrbitalSector115(t);CombineSector(t);}CreateLowMist();if(stage<2)terrain.position=Vector3.down*7;FrameCoast();UpdateCoast();Random.state=randomState;
 }
