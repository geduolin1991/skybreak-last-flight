...
public partial class SkyGame {
 IEnumerator VerifyCampaign(){
  ...
  Difficulty=0;Ship=0;foreach(string arg in Environment.GetCommandLineArgs())if(arg.StartsWith("--sky-ship="))Ship=Mathf.Clamp(int.Parse(arg.Substring(11)),0,2);AutoFire=true;MouseControl=false;researchFire=researchArmor=researchSkill=0;pilotXp[Ship]=0;
  ...
  while(Time.realtimeSinceStartup-start<450){
   ...
   if(SkillCooldown<=0&&Enemies.Count>0){UsePilotSkill();skillsUsed++;}
   ...
   if(nextDecision<=0){nextDecision=.1f;float best=float.MaxValue;Vector2 bestMove=Vector2.zero;
    ...
    if(target==null)foreach(var e in Enemies){float value=e.elite?100:20-Mathf.Abs(e.pos.x-PlayerPos.x)-Mathf.Abs(e.pos.z-PlayerPos.z)*.2f;if(e.pos.z>PlayerPos.z+2&&value>targetValue){targetValue=value;target=e;}}
    ...
    for(int x=-1;x<=1;x++)for(int z=-1;z<=1;z++){
     Vector2 m=Vector2.ClampMagnitude(new Vector2(x,z),1);Vector3 velocity=new Vector3(m.x,0,m.y)*(Ship==2?13.5f:Ship==1?10:12);
