...
public partial class SkyGame {
 ...
 void Shoot(bool focused){
  ...
  else FireRail((WeaponMode==0?80:31)*dmg*(focused&&CurrentRoute==0&&RouteTier>=2?1.15f:1),focused);
  RouteFire(dmg);OnWeaponFired();
 }
 void FireRail(float damage,bool focused){Vector3 start=PlayerPos+Vector3.forward*1.35f;Vector3 end=start+Vector3.forward*36;float width=WeaponMode==0?.23f:.1f;
  ...
  foreach(var e in Enemies.ToArray()){float radius=EnemyRadius(e);if(e.pos.z>=start.z-radius&&SegmentDistance(e.pos,start,end)<radius+(focused?.25f:.38f)){Burst(e.pos,10,2,.65f);DamageEnemy(e,damage);}}
 ...
 void WeaponImpact(Round b,Hostile direct){
  if(b.kind==4){float blast=Ship==1&&CurrentRoute==0?1.2f+(RouteTier-1)*.1f:1;Shockwave(b.pos,new Color(1,.7f,.28f),2.25f*blast,.27f);Burst(b.pos,15,1,.7f);foreach(var e in Enemies){if(e==direct||(e.pos-b.pos).sqrMagnitude>6.25f*blast*blast)continue;float d=b.damage*.6f;if(pendingDamage.ContainsKey(e))pendingDamage[e]+=d;else pendingDamage[e]=d;}if(impactGate<=0){Sound("MissileBurst",.34f);impactGate=.07f;}}
