...
public static class SkyBuild {
 ...
 if(!f.Contains("Frame")||o.GetComponentsInChildren<Transform>().Any(t=>t.name.StartsWith("Motion_")))CombineModel(o);PrefabUtility.SaveAsPrefabAsset(o,"Assets/Resources/Models/"+Path.GetFileNameWithoutExtension(f)+".prefab");UnityEngine.Object.DestroyImmediate(o);}
 ...
 static void CombineModel(GameObject root){
  ...
  foreach(var part in moving){part.SetParent(root.transform,true);CombinePart(part.gameObject,Array.Empty<Transform>(),root.name+"_");}
  CombinePart(root,moving);
 }
 static void CombinePart(GameObject root,Transform[] keep,string scope=""){
  ...
  foreach(var pair in groups){
   var mesh=new Mesh();mesh.indexFormat=UnityEngine.Rendering.IndexFormat.UInt32;mesh.CombineMeshes(pair.Value.ToArray(),true,true);mesh.name=scope+root.name+"_"+pair.Key.name;
