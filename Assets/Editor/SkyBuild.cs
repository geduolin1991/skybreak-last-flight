...
public static class SkyBuild {
 ...
 foreach(var f in Directory.GetFiles("Assets/Art/Models","*.fbx")){var prefab=AssetDatabase.LoadAssetAtPath<GameObject>(f);if(!prefab)throw new Exception("Missing model "+f);var o=(GameObject)PrefabUtility.InstantiatePrefab(prefab);PrefabUtility.UnpackPrefabInstance(o,PrefabUnpackMode.Completely,InteractionMode.AutomatedAction);foreach(var renderer in o.GetComponentsInChildren<Renderer>()){var src=renderer.sharedMaterials;for(int i=0;i<src.Length;i++){var original=src[i];string name=original?original.name:"Titanium_Dark";if(name.StartsWith("World_"))name=name.Substring(6);string path="Assets/Art/Materials/"+name+".mat";var mat=AssetDatabase.LoadAssetAtPath<Material>(path);if(!mat){mat=new Material(Shader.Find("Standard"));mat.name=name;Color c=new Color(.16f,.23f,.28f);bool emission=false;switch(name){case "Ceramic_Ivory":c=new Color(.65f,.76f,.79f);break;case "Titanium_Dark":c=new Color(.025f,.046f,.07f);break;case "Steel_Edge":c=new Color(.16f,.23f,.28f);break;case "Enemy_Red":c=new Color(.34f,.032f,.019f);break;case "Safety_Orange":c=new Color(.95f,.24f,.045f);break;case "Ion_Cyan":c=new Color(.04f,.75f,1);emission=true;break;case "Reactor_Orange":c=new Color(1,.12f,.015f);emission=true;break;case "Core_Violet":c=new Color(.54f,.055f,1);emission=true;break;case "Cockpit_Glass":c=new Color(.015f,.11f,.18f);break;}mat.color=c;mat.SetFloat("_Metallic",name=="Ceramic_Ivory"?.45f:.72f);mat.SetFloat("_Glossiness",name=="Cockpit_Glass"?.92f:.62f);if(emission){mat.EnableKeyword("_EMISSION");mat.SetColor("_EmissionColor",c*3);}mat.enableInstancing=true;AssetDatabase.CreateAsset(mat,path);}src[i]=mat;}renderer.sharedMaterials=src;}
