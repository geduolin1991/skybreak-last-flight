...
work=ROOT/'Build/VoiceWork1.13';work.mkdir(parents=True,exist_ok=True)
records_path=work/'generation.json'
records=json.loads(records_path.read_text()) if records_path.exists() else {}
bank=[c for c in json.loads((ROOT/'Assets/Resources/Voices/voice-bank.json').read_text())['clips'] if c['id'].startswith('frame113_')]
...
def signature(c,lang):
 ...
 return hashlib.sha256((c['text'] if lang=='zh' else c[lang]).encode()+ref.read_bytes()+b'frame-113-native-reading-1').hexdigest()
def done(c,lang):
 ...
 for lang,language in [('zh','Chinese'),('en','English'),('ja','Japanese')]:
  for pilot in range(3):
   ...
   for offset in range(0,len(entries),3):
    group=entries[offset:offset+3];mx.random.seed(11300+pilot*113+offset+list(refs).index(lang)*179)
...
print('SKYBREAK_FRAME_VOICES_COMPLETE',len(records),flush=True)
